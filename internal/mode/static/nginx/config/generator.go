...
// secretStorer stores Secret that contains the JWT for NGINX Plus usage reporting.
...
func (g GeneratorImpl) Generate(conf dataplane.Configuration) []file.File {
	// TODO(sberman): this length isn't accurate, there are way more files now
...
func (g GeneratorImpl) executeConfigTemplates(
	...
	if g.plus {
		plusFileCount = 3 // mgmt.conf, deployment_ctx.json, license.jwt
	}
...
// it writes the JWT and deployment context files that are referenced in the mgmt block.
func (g GeneratorImpl) generateMgmtFiles() []file.File {
	var files []file.File

	token := g.secret.GetJWT()
	tokenFile := file.File{
		Content: token,
	...
	return append(files, tokenFile, mgmtBlockFile)
}
