...
func TestGenerate(t *testing.T) {
	...
	// setup for Generating NGINX Plus mgmt files

	plus := true
	fakeSecret := &configfakes.FakeSecretStorer{}
	fakeSecret.GetJWTReturns([]byte("license"))
	...
	generator := config.NewGeneratorImpl(
		plus,
		&ngfConfig.UsageReportConfig{
			Endpoint: "test-endpoint",
		},
		fakeSecret,
		&ngfConfig.GatewayPodConfig{
			Name: "pod1",
		},
		fake.NewFakeClient(ngfPod, ngfReplicaSet, kubeNamespace, nodeList),
		ctlrZap.New(),
	)

	files := generator.Generate(conf)

	g.Expect(files).To(HaveLen(14))
	...
	*/
	...
	g.Expect(files[10]).To(Equal(file.File{
		Type:    file.TypeSecret,
		Path:    "/etc/nginx/secrets/license.jwt",
		Content: []byte("license"),
	}))

	g.Expect(files[11].Path).To(Equal("/etc/nginx/secrets/test-certbundle.crt"))
	certBundle := string(files[11].Content)
	g.Expect(certBundle).To(Equal("test-cert"))

	g.Expect(files[12]).To(Equal(file.File{
		Type:    file.TypeSecret,
		Path:    "/etc/nginx/secrets/test-keypair.pem",
		Content: []byte("test-cert\ntest-key"),
	}))

	g.Expect(files[13].Path).To(Equal("/etc/nginx/stream-conf.d/stream.conf"))
	g.Expect(files[13].Type).To(Equal(file.TypeRegular))
	streamCfg := string(files[13].Content)
