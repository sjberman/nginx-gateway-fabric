...
Secret before upgrading. Follow the steps in the [Before you begin](#before-you-begin) section to create the Secret, and specify the Secret name by setting `--set nginx.usage.secretName=nplus-license` when running `helm upgrade`.{{</important>}}
...
These two Secrets will be referenced in the helm installation command below.

{{<note>}}For more information on why this is needed and additional configuration options, see the [NGINX Plus Image and JWT Requirement]({{< relref "installation/nginx-plus-jwt.md" >}}) document.{{</note>}}
...
{{< note >}}The `nginx.usage.secretName` value is set to the NGINX Plus Secret that was created in the [Before you begin](#before-you-begin) section.{{< /note >}}

To install the latest stable release of NGINX Gateway Fabric in the **nginx-gateway** namespace, run the following command:

```shell
helm install ngf oci://ghcr.io/nginxinc/charts/nginx-gateway-fabric  --set nginx.image.repository=private-registry.nginx.com/nginx-gateway-fabric/nginx-plus --set nginx.plus=true --set nginx.usage.secretName=nplus-license --set serviceAccount.imagePullSecret=nginx-plus-registry-secret --create-namespace -n nginx-gateway
```
...
```shell
helm install ngf . --set nginx.image.repository=private-registry.nginx.com/nginx-gateway-fabric/nginx-plus --set nginx.plus=true --set nginx.usage.secretName=nplus-license --set serviceAccount.imagePullSecret=nginx-plus-registry-secret --create-namespace -n nginx-gateway
```
...
- To upgrade from NGINX OSS to NGINX Plus, update the Helm command to include the necessary values for Plus:
  ...
  ```shell
  helm upgrade ngf oci://ghcr.io/nginxinc/charts/nginx-gateway-fabric  --set nginx.image.repository=private-registry.nginx.com/nginx-gateway-fabric/nginx-plus --set nginx.plus=true --set serviceAccount.imagePullSecret=nginx-plus-registry-secret  --set nginx.usage.secretName=nplus-license -n nginx-gateway
  ```
